...
*
##  File: Image.php (SKFrame_Ext_Image)
*
******************************************************************/
class SKFrame_Ext_Image
{    
    private $thumbwidth;//缩略图宽
    private $thumbheight;//缩略图高
    private $maxthumbwidth;//缩略图最大宽
    private $maxthumbheight;//缩略最大图高
    private $watermarkpos;//1:顶端居左|2:顶端居右|3:底端居左|4:底端居右|否则随机
    public $watermarkfile;//水印图片

    /**
    * 构造函数
    */
    function __construct($wf, $tw = 60, $th = 60, $mtw = 300, $mth = 300){
        $this->watermarkfile = $wf;
        $this->thumbwidth = $tw;
        $this->thumbheight = $th;
        $this->maxthumbwidth = $mtw;
        $this->maxthumbheight = $mth;
    }
    //生成缩略图    
    ...
    //图片水印
    function makewatermark($srcfile,$pos=4) {
        $this->watermarkpos = $pos;
	    //水印图片
        if(!file_exists($this->watermarkfile) || !$water_info = getimagesize($this->watermarkfile)) {
    	    return '';
        }
        ...
        $water_im = '';
        switch($water_info[2]) {
            case 1:@$water_im = imagecreatefromgif($this->watermarkfile);break;
            case 2:@$water_im = imagecreatefromjpeg($this->watermarkfile);break;
            case 3:@$water_im = imagecreatefrompng($this->watermarkfile);break;
            default:break;
        }
	    if(empty($water_im)) {
		    return '';
