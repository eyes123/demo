...
class ProjectModel extends Model
{
    ...
    public function getProjectList($limit)
    {
        ...
        $filed = array("yes_project.*,count(distinct(yes_app.id)) as app_number,count(distinct(yes_order.id)) as order_number");
...
//        echo $this->getLastSql();exit;
    ...
    //查询课程
    public function getProjectName($where)
    {
        $row = $this->field(array("id,project_name"))->where($where)->select();
//        echo $this->getLastSql();exit;
        return $row[0];
    }
