...
{
	return Think::trace($value, $label, $level, $record);
}
...
function A($name, $layer = '', $level = '')
{
	...
	$filepath = parse_res_name($name, $layer, $level);
	include APP_PATH . $filepath . EXT;
	$class = ucfirst($name) . ucfirst($layer);
	if (class_exists($class)) {
		$action = new $class();
...
function session($name, $value = '')
{
	...
	if (is_array($name)) { // session初始化 在session_start 之前调用
		...
		if (C('SESSION_TYPE')) { // 读取session驱动
			$type = C('SESSION_TYPE');
			$class = strpos($type, '\\') ? $type : 'Think_Session_Driver_' . ucwords(strtolower($type));
			$hander = new $class();
